...
int main(int argc, char* argv[])
{
    ...
    // source = src
    // mask = target
    // inpaint_me = target in BGR2Lab
    // Mat objects to store images. mask = target
    ...
    // Computing gradients
    // I don't know why GaussianBlur = Smoothing filter is used here
    // This smooths the source image
    GaussianBlur( src, src, Size(3,3), 0, 0, BORDER_DEFAULT);
    
    // Converts the source image(BGR) to grayscale image
